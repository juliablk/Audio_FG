...
scores <- scores %>%
  dplyr::group_by(station) %>% arrange(file_timestampsec) %>%
...
#############################################################################################
# StartEnd_df <- StartEnd_df2
# StartEnd_df2 <- StartEnd_df
# StartEnd_df <- StartEnd_df[-203:-314,]
# seqs <- unique(StartEnd_df$seq)
...
for (j in 1:length(stations)) {
  ...
  temp.j <- subset(temp, WA.Song.Meter.Prefix == station.j)
   ...
   for (i in 1:length(seqs)) {
    ...
    # path = paste0("C:/Users/Beheerder/Documents/UU/A GSLS/MinPr/R/Audio2023/SubsetDeployments/", TR, "/", station.j,"_", filetimestamp.i , ".wav")
    ...
    if (file.exists(path)) {
      ...
      # als de vorige clip op einde zat en dees op begin mergen
      ...
      if (i > 1 && secs_start.i - secs_end.prev <= 10) {
        ...
        # als de vorige clip niet <= 10 sec verschilt niet mergen en Timestamps weer op 0
      } else {
        Timestamps <- c()
      }

      # als de volgende clip op het begin zit en dees op het einde niet opslaan
      if (i + 1 <= length(seqs) && secs_start.next - secs_end.i <= 10) {
        Timestamps <- c(Timestamps, timestamp.i)
        previous_clip <- wav_file
        previous_length <- length
      } else {
        # create new directory for the clipped files, with timestamp including seconds
        TR.group <- substr(TR, 1, 4)
        dir <- paste0("E:/Triangulation 2023/", TR.group, "_clipped_fullwv") # dir if everything is stored in one file
        # dir <- paste0("D:/Triangulation 2023/", TR.group, "_clipped/subset0405", timestamp.i) #dir if clips are stored per timestamp
        # dir <- paste0("C:/Users/Beheerder/Documents/UU/A GSLS/MinPr/R/Audio2023/", TR.group, "_clipped/subset0405min85/", timestamp.i)
  ...
  for (i in 1:length(seqs)) {
    ...
    if (file.exists(path)) { # if there is indeed a file
      ...
      if (i + 1 <= length(seqs) && secs_start.next - secs_end.i <= 10) {
        Timestamps <- c(Timestamps, timestamp.i)
        previous_length <- length
      } else { # dus als er geen aansluitende clip is opslaan:
...
filetimestamp.i <- "20230410_014502"
temperature <- temp$Temperature.Int[temp$file_timestamp == filetimestamp.i]



saveRDS(df_durations2, "C:/Users/Beheerder/Documents/UU/A GSLS/MinPr/R/Audio2023/lengthsTRA.rData")
readRDS("C:/Users/Beheerder/Documents/UU/A GSLS/MinPr/R/Audio2023/lengthsTRA.rData")


write_csv(df_durations2, "D:/Triangulation 2023/lengthsTRA_csv.csv")
df <- loadRDS("D:/Triangulation 2023/lengthsTRA.rData")
df <- loadRDS("C:/Users/Beheerder/Documents/UU/A GSLS/MinPr/R/Audio2023/lengthsTRA.rData")
wd
df$LENGTH <- as.numeric(df$LENGTH)
df <- as.data.frame(df)

long <- subset(df, LENGTH > 500)


####################################################################################################


# even testfiles knippen


library(data.table)

#load scores from TRA 
scores <- readRDS("E:/Triangulation 2023/scores.rData")
stations <- (unique(scores$station))
stations <- stations[!is.na(stations)]


testpoints <- list.files("E:/test_points", full.names = T)
selectiontables.list <- testpoints[testpoints %like% "ALOU"]

# list with all selectiontables
selectiontables <- lapply(selectiontables.list, read.delim)
...
for (i in 1:length(selectiontables)) {
  df <- selectiontables[[i]]
  min.begin <- min(df$Begin.Time..s.)
  min.end <- min(df$End.Time..s.)

  # check hoeveelste rec dit is, 4:57 sec in elke file
  nfile <- min.begin / 297

  # pak alleen het hele getal
  nfile <- as.numeric(substr(nfile, 1, 1))

  # get the residual seconds
  start <- min.begin - nfile * 297
  end <- min.end - nfile * 297

  # substract 30 seconds before start and add 20 after end
  start.i <- start - 10
  end.i <- end + 10
  timestamp <- timestamp <- sub(".*\\\\(\\d+_\\d+)\\\\.*", "\\1", df$Begin.Path)[1]
...
startend <- as.data.frame(cbind(Time, Starts, Ends))
startend$Starts <- as.numeric(startend$Starts)
startend$Ends <- as.numeric(startend$Ends)

#if we have a negative start we set it to 0
startend$Starts[startend$Starts<0] <- 0

#if the end value is greater than 297 we set it to 297
startend$Ends[startend$Ends>=297] <- 297
...
for (j in 1:length(stations)) {
  ...
  for (i in 1:nrow(startend)) {
    print(i)
    filetimestamp.i <- startend$Time[i]

    # Read the .WAV file into R
    path <- paste0("E:/test_points/Grouped/", filetimestamp.i, "/", station.j, "_", filetimestamp.i, ".wav")


    if (file.exists(path)) {
      # Set the start and end times in seconds
      start.time <- as.numeric(startend$Starts[startend$Time == filetimestamp.i])
      end.time <- as.numeric(startend$Ends[startend$Time == filetimestamp.i])
      length <- end.time - start.time

      wav_file <- readWave(path, from = start.time, to = end.time, units = "seconds")


      # create new directory for the clipped files
      dir <- paste0("E:/test_points/Grouped/_clipped_fullwv/", filetimestamp.i) # dir if everything is stored in one file
      if (!dir.exists(dir)) dir.create(dir, recursive = T)

      path2 <- paste0(dir, "/", station.j, "_", filetimestamp.i, "_clpd.wav")
      print(path2)
      ...
      comnd <- paste0("sox \"", path2, "\" \"", output_name, "\" rate 4k")
      ...
      LENGTH <- c(LENGTH, length)
      PATH <- c(PATH, output_name)
    }
  }
}

Length_testpoints <- as.data.frame(cbind(LENGTH, PATH))

saveRDS(Length_testpoints, "E:/test_points/lengths_testpointsA10.rData")
