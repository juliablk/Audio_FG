...
Power<- data.frame(File_audiostats$`Begin File`, File_audiostats$`Avg Power Density (dB FS/Hz)`,
...
saveRDS(PD_above1500, "E:/Triangulation 2023/PowerDistance_ABCDE.rData")
saveRDS(temp_lengths, "E:/Triangulation 2023/templengths_ABCDE.rData")


PD_above1500 <- readRDS("E:/Triangulation 2023/PowerDistance_ABCDE.rData")

temp_lengths <- readRDS( "E:/Triangulation 2023/templengths_ABCDE.rData")
...
File_audiostats1<- imp_raven(path=path1,all.data = TRUE, freq.cols = FALSE,
                                                        warbler.format=T, name.from.file = TRUE,
                                                        ext.case ="lower")

if (dir.exists(path2)){

File_audiostats2<- imp_raven(path=path2,all.data = TRUE, freq.cols = FALSE,
                             warbler.format=T, name.from.file = TRUE,
                             ext.case ="lower")

File_audiostats <- rbind(File_audiostats1, File_audiostats2)} else {
...
wv <- tuneR::readWave(filename = X$`Begin Path`, 
                          header = FALSE, units = "seconds", from = X$start, 
                          to = X$end, toWaveMC = F)


bit_ranges <- data.frame(bit = c(1, 8, 16, 24, 32, 
                                 64), low = c(-1, 0, -32767, -8388607, -2147483647, 
                                              -1), high = c(1, 254, 32767, 8388607, 2147483647, 
                                                            1))
bit_range <- bit_ranges[bit_ranges$bit == wv@bit, 
                        c("low", "high"), drop = TRUE]
...
#these points should have been removed during the distance to middle rec filter... 
#thrying that againg
